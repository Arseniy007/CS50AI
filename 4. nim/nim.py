...
class NimAI():
    ...
    def update_q_value(self, state: list, action: tuple, old_q, reward, future_rewards) -> None:
        """
    ...
    def best_future_reward(self, state: list):
        ...
        `state`, return 0.
        """
        # Get all possible actions for given state
        possible_actions = Nim.available_actions(state)

        # Find highest possible reward
        best_reward = 0
    ...
    def choose_action(self, state: list, epsilon=True) -> tuple:
        ...
        """
        # Get all possible actions for given state
        possible_actions: set = Nim.available_actions(state)
        ...
        # Calculate best possible action for current state
        best_reward = -1
        best_action = tuple()
        for action in possible_actions:
            reward = self.get_q_value(state, action)
